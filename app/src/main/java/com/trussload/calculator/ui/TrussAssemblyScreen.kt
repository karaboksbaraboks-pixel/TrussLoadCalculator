...
data class AssemblyElement(
    ...
    // Допустимая распределённая нагрузка
...
data class TrussAssembly(
    ...
    val elements: List<AssemblyElement> =
        emptyList(),

    val connections: List<AssemblyConnection> =
        emptyList(),
    ...
    // --------------------------------------------------------
    // КОЛИЧЕСТВО БЛОКОВ / СОЕДИНИТЕЛЕЙ
    // --------------------------------------------------------
...
//
// При начале свободного перемещения удаляем его старые стыки.
// После отпускания snap создаст новый стык.
// ============================================================
